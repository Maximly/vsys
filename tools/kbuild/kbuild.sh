...
echo "execute_process(" >>$kbuild_cmake
echo '    COMMAND bash -c "echo ${PROJECT_NAME} | sed -e \"s/\\(.\\)/'"'"'\\1'"'"',/g\" | sed -e \"s/\\(.*\\),/{ \\1, 0 }/\""' >>$kbuild_cmake
echo "    OUTPUT_VARIABLE KBUILD_MODNAME2" >>$kbuild_cmake
echo "    OUTPUT_STRIP_TRAILING_WHITESPACE)" >>$kbuild_cmake
echo 'target_compile_definitions(${PROJECT_NAME} PRIVATE KBUILD_MODNAME2=${KBUILD_MODNAME2})' >>$kbuild_cmake
...
add_header_path() {
    path=$(dirname $1)
    while [ "$path" != "" ]; do
        echo '${CMAKE_SOURCE_DIR}/inc/targets/linux/kernel/generated/'$path >>$kbuild_cmake_tmp
        path1=$path
        path=${path%/*}
        if [ "$path" = "$path1" ]; then
            path="";
...
target_include_directories(${PROJECT_NAME} BEFORE PRIVATE
    ${CMAKE_SOURCE_DIR}/inc/targets/linux/kernel/generated' >>$kbuild_cmake
